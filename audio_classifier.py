...
class AudioClassifier:
    ...
    def load(self):
        """Import TensorFlow and load YAMNet (downloads on first run)."""
        self._ensure_model()
    ...
    def _ensure_model(self):
        ...
        # YAMNet on a 1.5 s window is tiny; don't let TF's default thread pools
        # (one per core) compete with the video pipeline for CPU.
        try:
    ...
    def classify(self, waveform: np.ndarray) -> dict:
        ...
        music = float(mean_scores[list(_MUSIC_CLASS_IDS)].max())
        speech = float(mean_scores[list(_SPEECH_CLASS_IDS)].max())
