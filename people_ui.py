...
from PyQt5.QtWidgets import (
    ...
    QLineEdit, QDialogButtonBox, QFormLayout, QGroupBox, QSlider,
    QScrollArea, QFrame,
)
...
# Length of a voice sample recorded from the mic (must be ≤ audio_capture.BUFFER_SECONDS).
_RECORD_SECONDS = 5.0
...
class _EmbedThread(QThread):
    ...
    def run(self):
        try:
            ...
            recognizer = get_shared_recognizer(self._store)
...
class EditProfileDialog(QDialog):
    ...
    def __init__(self, profile: Profile | None, store: ProfileStore,
        ...
        self._record_btn = QPushButton(f"Record {_RECORD_SECONDS:.0f} s from mic")
    ...
    def _refresh_voice_list(self):
        ...
        for i, _ in enumerate(self._pending_voice_recordings):
            item = QListWidgetItem(f"Recording {i+1}  (new, {_RECORD_SECONDS:.0f} s)")
    ...
    def _on_record_voice(self):
        if self._audio_grabber is None:
            return
        # Let the capture ring buffer fill for the sample length, then pull
        # that many seconds out of it.  The buffer is 6 s (audio_capture), so
        # a 5 s sample is real, not truncated.
        self._record_btn.setEnabled(False)
        self._record_btn.setText("Recording…")
        self._record_remaining_s = _RECORD_SECONDS
    ...
    def _on_record_complete(self):
        ...
        if self._record_btn is not None:
            self._record_btn.setText(f"Record {_RECORD_SECONDS:.0f} s from mic")
    ...
    def _remove_existing_image(self, filename: str):
        ...
        if QMessageBox.question(
            self, "Remove image", "Remove this reference image?",
            QMessageBox.Yes | QMessageBox.No
        ) != QMessageBox.Yes:
            return
...
class PeopleWindow(QMainWindow):
    ...
    def __init__(self, store: ProfileStore,
        ...
        # Audio capture is controlled from the main panel (one source of truth
        # for the device + enabled state); recording here just reads from it.
        if self._audio_thread is not None:
            self._audio_hint = QLabel("")
            self._audio_hint.setStyleSheet("color: #b58900; font-size: 11px;")
            layout.addWidget(self._audio_hint)
    ...
    def _refresh_audio_hint(self):
        if self._audio_thread is None:
            return
        if self._audio_thread.is_capturing:
            self._audio_hint.setText("")
        else:
            self._audio_hint.setText(
                "Audio is off — enable it in the main panel to record voice samples from the mic.")
    ...
    def _grab_audio(self, seconds: float) -> np.ndarray | None:
        ...
        if self._audio_thread is None:
            return None
        return self._audio_thread.get_recent_audio(seconds)
