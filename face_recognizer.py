...
_shared: "FaceRecognizer | None" = None


def get_shared_recognizer(store: ProfileStore) -> "FaceRecognizer":
    """Return the process-wide FaceRecognizer, creating it on first use.

    The buffalo_l model set is ~300 MB in memory and takes seconds to load, so
    the live recognition worker and the People window's embedding jobs share
    one instance instead of each loading their own copy.
    """
    global _shared
    with _shared_lock:
        if _shared is None or _shared._store is not store:
            _shared = FaceRecognizer(store)
        return _shared
...
class FaceRecognizer:
    ...
    def has_profiles(self) -> bool:
        """True if at least one profile has face embeddings to match against."""
