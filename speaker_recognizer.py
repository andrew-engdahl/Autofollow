...
_shared: "SpeakerRecognizer | None" = None


def get_shared_recognizer(store: ProfileStore) -> "SpeakerRecognizer":
    """Return the process-wide SpeakerRecognizer (one ECAPA model for everyone)."""
    global _shared
    with _shared_lock:
        if _shared is None or _shared._store is not store:
            _shared = SpeakerRecognizer(store)
        return _shared
...
class SpeakerRecognizer:
    ...
    def has_profiles(self) -> bool:
        """True if at least one profile has voice embeddings to match against."""
