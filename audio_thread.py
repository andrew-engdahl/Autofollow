...
class AudioThread(QThread):
    ...
    def __init__(self, profile_store: ProfileStore, parent=None):
        ...
        self._capture.set_error_callback(lambda msg: self.error.emit(msg))
    ...
    def set_enabled(self, enabled: bool):
        ...
        if enabled:
            if not self._capture.start(self._device_index):
                self._enabled = False
                self.error.emit("Could not open audio input")
    ...
    def get_recent_audio(self, seconds: float) -> np.ndarray | None:
        ...
        if not self._capture.is_running:
            return None
        return self._capture.get_recent(seconds)

    def stop(self):
        self._running = False
        self._capture.stop()
        # A tick can be stuck inside a first-time model download; don't let
        # that hold up application shutdown.
        ...
        if self._classifier_failed:
            ...
            try:
                ...
                if not self._classifier.is_loaded:
                    ...
                    self.status.emit("Music classifier ready")
    ...
    def _run_speaker_recognition(self, window: np.ndarray) -> bool:
        if self._speaker_rec is None:
            ...
            try:
                ...
                self._speaker_rec = get_shared_recognizer(self._store)
            except Exception as e:
                print(f"SpeakerRecognizer init failed: {e}")
                self._speaker_rec_failed = True
                return False
        if not self._speaker_rec.has_profiles:
            return False
        if not self._speaker_rec.is_loaded:
